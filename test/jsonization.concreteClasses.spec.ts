...
test("Extension round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Extension"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.extensionFromJsonable(jsonable);
...
test("Extension deserialization fail", () => {
  for (const causeDir of TestCommon.findImmediateSubdirectories(
    path.join(TestCommon.TEST_DATA_DIR, "Json", "Unexpected", "Unserializable")
  )) {
    // NOTE (mristin):
...
test("Extension verification fail", () => {
  for (const causeDir of TestCommon.findImmediateSubdirectories(
    path.join(TestCommon.TEST_DATA_DIR, "Json", "Unexpected", "Invalid")
  )) {
    const clsDir = path.join(causeDir, "Extension");
    if (!fs.existsSync(clsDir)) {
      // NOTE (mristin):
...
test("Qualifier round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Qualifier"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.qualifierFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("AssetInformation round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "AssetInformation"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.assetInformationFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("Resource round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Resource"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.resourceFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("SpecificAssetId round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "SpecificAssetId"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.specificAssetIdFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("Submodel round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Submodel"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.submodelFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("RelationshipElement round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "RelationshipElement"),
      ".json"
    )
  );
  pths.sort();
...
test("SubmodelElementList round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "SubmodelElementList"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.submodelElementListFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("Property round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Property"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.propertyFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("MultiLanguageProperty round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "MultiLanguageProperty"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.multiLanguagePropertyFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("Range round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Range"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.rangeFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("ReferenceElement round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "ReferenceElement"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.referenceElementFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("Blob round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Blob"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.blobFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("File round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "File"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.fileFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("Entity round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Entity"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.entityFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("EventPayload round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "EventPayload"),
      ".json"
    )
  );
  pths.sort();
...
test("BasicEventElement round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "BasicEventElement"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.basicEventElementFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("Operation round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Operation"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.operationFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("OperationVariable round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "OperationVariable"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.operationVariableFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("Capability round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Capability"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.capabilityFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("ConceptDescription round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "ConceptDescription"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.conceptDescriptionFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("Reference round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Reference"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.referenceFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("Key round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Key"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.keyFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("LangStringNameType round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "LangStringNameType"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.langStringNameTypeFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("LangStringTextType round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "LangStringTextType"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.langStringTextTypeFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("Environment round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "Environment"),
      ".json"
    )
  );
  pths.sort();
...
test("LevelType round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "LevelType"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.levelTypeFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("ValueReferencePair round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "ValueReferencePair"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.valueReferencePairFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
...
test("ValueList round-trip OK", () => {
  const pths = Array.from(
    TestCommon.findFilesBySuffixRecursively(
      path.join(TestCommon.TEST_DATA_DIR, "Json", "Expected", "ValueList"),
      ".json"
    )
  );
  pths.sort();
  ...
  for (const pth of pths) {
    ...
    const instanceOrError = AasJsonization.valueListFromJsonable(jsonable);
    expect(instanceOrError.error).toBeNull();
    const instance = instanceOrError.mustValue();
